...
export function cleanProductDescriptionText(raw, fallbackName = '') {
  if (raw == null || raw === '') {
...
export function looksLikeCorruptProductName(name) {
...
export function getCleanProductName(product) {
...
function buildPerfumeShortDescriptionClient({ name, brand }) {
...
function stripAdminEditorFooterClient(html) {
  return String(html || '')
...
function normalizeImportedPerfumeShortClient(product, name) {
  const raw = String(product?.shortDescription || '').trim();
  if (!/imported perfume/i.test(raw)) return null;
  if (!/Rs\.?\s*[\d,]+/i.test(raw) && !/\d+\+\s*\./i.test(raw) && !/—\s*\d+\+/.test(raw)) {
    return null;
  }

  const brandMatch = raw.match(/^(.+?)\s+imported perfume/i);
  ...
  const brand = brandMatch ? brandMatch[1].trim() : brandTag ? String(brandTag).trim() : '';

  return buildPerfumeShortDescriptionClient({ name, brand });
}

export function getCleanShortDescription(product) {
  const name = getCleanProductName(product);
  if (bothFieldsAreImportJson(product)) return name;

  const perfumeShort = normalizeImportedPerfumeShortClient(product, name);
...
export function getCleanLongDescription(product) {
  const name = getCleanProductName(product);
...
export function formatDescriptionAsHtml(plainText) {
...
export function resolveProductDescriptionHtml(product) {
  ...
  if (looksLikeSafeAdminHtml(raw)) return stripAdminEditorFooterClient(raw);
  return formatDescriptionAsHtml(getCleanLongDescription(product));
}

export function sanitizeProductDescriptions(product) {
